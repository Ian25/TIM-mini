...
";
        ...
        */
        ...
        void AdjustAmounts()
        {
            ...
            {
                List<MyInventoryItem> stack = new List<MyInventoryItem>();
                inven.GetItems(stack);
                foreach (MyInventoryItem s in stack)
                {
                    itype = "" + stack.Type.TypeId;
                    itype = itype.Substring(itype.LastIndexOf('_') + 1).ToUpper();
                    isub = stack.Type.SubtypeId.ToString().ToUpper();

                    amount = (long)((double)s.Amount * 1e6);
                    typeAmount[itype] -= amount;
        ...
        */
        ...
        void AddInvenRequest(IMyTerminalBlock block, int inv, string itype, string isub, int priority, long amount)
        {
            ...
            if (((block is IMyGasGenerator | block is IMyReactor | block is IMyRefinery | block is IMyUserControllableGun) & inven.Owner != null) && block.GetUseConveyorSystem())
            {
